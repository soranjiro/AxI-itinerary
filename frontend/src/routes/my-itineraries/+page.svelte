	...
	<div class="container mx-auto px-4 py-8 max-w-6xl">
		...
		<div class="mb-8">
			<div class="flex items-center justify-between mb-6">
				...
				<button
					...
					class="flex items-center space-x-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg"
				>
					<Plus class="w-5 h-5" />
					<span>新しいしおりを作成</span>
				</button>
		...
		{:else if itineraries.length === 0}
			<div class="text-center py-16">
				<BookOpen class="w-24 h-24 text-gray-300 dark:text-gray-600 mx-auto mb-6" />
				<h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">まだしおりがありません</h2>
				<p class="text-gray-600 dark:text-gray-300 mb-8 max-w-md mx-auto">
					最初の旅行しおりを作成して、素晴らしい旅の思い出を残しましょう
				</p>
				<button
					on:click={createNewItinerary}
					class="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-semibold py-4 px-8 rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg"
				>
					<Plus class="w-6 h-6" />
					<span>最初のしおりを作成</span>
				</button>
		...
		{:else}
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{#each itineraries as itinerary}
					<button
						class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/50 p-6 hover:shadow-2xl transition-all duration-300 transform hover:scale-105 cursor-pointer text-left w-full"
					...
					>
						<div class="flex items-start justify-between mb-4">
							...
							<div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center flex-shrink-0 ml-4">
